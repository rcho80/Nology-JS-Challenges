...
export const getData = (url) => {
  ...
  const response = getURL.then(response => {
...
export const getNames = (url) => {
  ...
  const response = fetchURL.then(response => {
    return response.json();
  })
  const mapNames = response.then(data => {
...
export const getEmployedPeople = (url) => {
  ...
  const filtArr = response.then(people => {
    return people.filter(employee => {
      return employee.isEmployed === true
    })
  });
  return filtArr;
}
...
export const findPersonWithId = (url, id) => {
  ...
  const checkResponse = response.then(person => {
    return person.map(item => {
      return id === item.id ? item : "Person not found";
    })
