...
export const calculateLifetimeSupply = (snickersPerDay, age, maxAge) => {
  let howMany = snickersPerDay * 365 * (maxAge - age)
  return howMany;
};
...
export const calculateAreaOfCirlce = (radius) => {
  const areaCalc = (Math.PI * radius ** 2); // dont use .tofixed() as it returns a string and need to then use parseFloat...
  return Math.round(areaCalc * 100) / 100; // use this formulat to keep type as numbers and less code to work out
};
...
//better to use switch statement for this one
// always start with error parameters before starting the logic of code => get these out of the way 
// if (grade.length !== 1) ==> if the score is not one length minimum