...
export class Counter {
  ...
  // return this.count > 0 ? --this.count : this.count; below can use
  decrement() {
    if (this.count > 0) {
...
export class Engine {
  ...
  // //  The second is called stopEngine().
  // *If engineIsRunning is true it will set engineIsRunning to false and
  // //  return "Engine has stopped running".
  // *If engineIsRunning is false it will
...
export class BankAccount {
  ...
  // when looking at this ==> start with the negative first 
  // i.e. do "Invalid input, unable to deposit" first
  // get a valid number first - positive and number 

  deposit(toDeposit) {
    let parsed = parseFloat(toDeposit);
  ...
  withdraw(toWithdraw) {
    let parsed = parseFloat(toWithdraw);

    if (parsed !== NaN && parsed > this._balance) {
