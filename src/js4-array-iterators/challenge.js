...
export const createPercentageList = (numbersArr) => {
  return numbersArr.map(item => `${item * 100}%`);
};
...
export const convertStringToNumbersArray = (numberString) => {
  ...
  const filtered = splitString.map(item => {
    return Number(item);
  })
  return filtered;
}
...
export const createOddEvenArray = (numberString) => {
  const splitString = numberString.split("+");
  const list = splitString.map(item => {
    if (item % 2 === 0) {
      return "even"
    }
    return "odd"
...
export const filterBooksBySearch = (booksArr, searchTerm) => {
  const stringBooks = booksArr.filter(item => {
...
export const formatStringArray = (stringArr) => {
  const list = stringArr.map(str => str.trim().toLowerCase());
  return list.join("+");
};
...
export const formatString = (string) => {
  ...
  const newReg = string.replace(regex, '');
  const newArr = Array.from(newReg);
  return newArr.map((item, index) => index % 2 === 0 ? item.toUpperCase() : item.toLowerCase())
}
 ...
 */
//When used as a function, Number(value) converts a string or other value (array here) to the Number type. If the value can't be converted, it returns NaN.// const numArr = mixedArray.map(Number); // this returns [-1, NaN, 3, 5, 15, 2, 0]
export const fizzBuzz = (mixedArray) => {
  const list = [];
  const regex = /^[0-9]+$/g;

  const numArr = mixedArray.map(Number);
  const filtered = numArr.filter((item) => {
...
//   const x = n % 3 === 0;
//   const y = n % 5 === 0;
//   if (x && y) return "FizzBuzz";
//   else if (x) return "Fizz";
//   else if (y) return "Buzz";
...
//   //     .map(fizzBuzzString);
//   return mixedArray.reduce((acc, value) => {
//     if (value > 0) {
//       const parsed = Number(value);
//       acc.push(fizzBuzzString(parsed));
//     }
//     return acc;
//   }, []);
//   // [] => [] => ["Fizz"]
