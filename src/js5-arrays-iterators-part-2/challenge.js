...
export const totalScoresArr = (scoresArr) => {
  const reduceArr = scoresArr.reduce((acc, currVal) =>
    acc + currVal, 0)
  return reduceArr;
};
...
export const reverseString = (toReverse) => {
  ...
  // const arrStr = Array.from(toReverse);
  // const revArr = arrStr.reverse().join();
  // const strArr = revArr.split(",").join("");
  // return `${strArr}`;
};
...
export const sortCharactersAlphabetically = (charcterArr) => {
  const charAlph = charcterArr.map((item) => {
    return item.toLowerCase();
  })
  return Array.from(charAlph).sort();
};
...
export const sortNumbersHighToLow = (numberArr) => {
  const mapArr = numberArr.sort((first, second) => second - first)
  return mapArr;
};
...
// includes / find for lists complicated
...
export const checkItemInstock = (toCheck) => {
  ...
  const checkList = stockList.indexOf(toCheck);
  if (checkList === -1) {
    return `Sorry ${toCheck} is not instock.`;
  } else {
    return `${toCheck} is instock, it is on aisle ${checkList}.`
  }
};
...
export const checkPrimaryColours = (coloursArr) => {
  ...
  const everyArr = coloursArr.every(elem => {
    //this pipe expression is limited to only what is currently being checked ==> otherwise use includes
    return elem === primary[0] || elem === primary[1] || elem === primary[2]; // can use includes() here 
  })
  return everyArr;
};
...
export const checkStringPalindrome = (stringOne) => {
  const strArr = stringOne.split("").reverse().join("");
  if (strArr === stringOne) {
    return true;
  }
  return false;
...
export const totalNestedScoresArr = (scoresArr) => {
  const list = [];
  const finalList = [];
  const mapArr = scoresArr.map(item => {
    list.push(item);
  });
  for (let i = 0; i < list.length; i++) {
    const reduced = list[i].reduce((acc, curr) => {
      return acc + curr;
    });
    finalList.push(reduced)
  }
  return finalList;
};
...
export const encryptString = (toEncrypt) => {
  ...
  const arrayed = Array.from(toEncrypt);
  ...
  for (let i = 0; i < arrayed.length; i++) {
    totalArr[i % 3].push(arrayed[i]);
  }

  for (let j = 0; j < totalArr.length; j++) {
    const redArr = totalArr[j].reduce((acc, curr, index) => {
      return acc + curr;
    }, "")
    list.push(redArr);
  }
...
//starting value here within reduced can be anything (i.e. here could place totalArr as this value and then)
//acc is important to note the value based will take value of first index if nothing is passed